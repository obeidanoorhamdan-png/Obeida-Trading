...
function initTimeframeDetectionV2() {
    ...
    // دالة مخصصة لتحديث الواجهة بالفريم
    function updateTimeframeUI() {
        // تحديث عنصر الفريم الرئيسي
    ...
    // =====================================================
    // ========== الاستراتيجيات المحسّنة (120+ استراتيجية) ==========
    // =====================================================
    ...
    function strategy_RSI(candles) {
        ...
        if(rsi < 30) return {signal:"CALL", confidence: 94, strength: "قوية جدا", reason: `RSI ${rsi.toFixed(0)} - تشبع بيعي`};
        if(rsi > 70) return {signal:"PUT", confidence: 94, strength: "قوية جدا", reason: `RSI ${rsi.toFixed(0)} - تشبع شرائي`};
        return null;
    }
    ...
    function strategy_Stochastic(candles) {
        ...
        if(k < 20 && k > prevK) return {signal:"CALL", confidence: 89, strength: "قوية", reason: "ستوكاستيك تشبع بيعي متجه صعود"};
        if(k > 80 && k < prevK) return {signal:"PUT", confidence: 89, strength: "قوية", reason: "ستوكاستيك تشبع شرائي متجه هبوط"};
        return null;
    }
    ...
    function strategy_Momentum(candles) {
        ...
        if(mom1 > 0 && mom1 > mom2 && mom2 > mom3) return {signal:"CALL", confidence: 86, strength: "قوية", reason: "زخم صاعد متسارع"};
        if(mom1 < 0 && mom1 < mom2 && mom2 < mom3) return {signal:"PUT", confidence: 86, strength: "قوية", reason: "زخم هابط متسارع"};
        return null;
    }
    ...
    function strategy_WilliamsR(candles) {
        ...
        if(wr < -80 && wr > prevWr) return {signal:"CALL", confidence: 88, strength: "قوية", reason: "Williams %R تشبع بيعي"};
        if(wr > -20 && wr < prevWr) return {signal:"PUT", confidence: 88, strength: "قوية", reason: "Williams %R تشبع شرائي"};
        return null;
    }
    ...
    function strategy_CCI(candles) {
        ...
        if(cci < -100 && cci > prevCci) return {signal:"CALL", confidence: 85, strength: "قوية", reason: "CCI ارتداد من -100"};
        if(cci > 100 && cci < prevCci) return {signal:"PUT", confidence: 85, strength: "قوية", reason: "CCI ارتداد من +100"};
        return null;
    }
    ...
    function strategy_BollingerBands(candles) {
        ...
        if(current < lower && current > prev) return {signal:"CALL", confidence: 90, strength: "قوية جدا", reason: "ارتداد من بولينجر السفلي"};
        if(current > upper && current < prev) return {signal:"PUT", confidence: 90, strength: "قوية جدا", reason: "ارتداد من بولينجر العلوي"};
        return null;
    }
    ...
    function strategy_MACD(candles) {
        ...
        if(candles.length > 27) {
            ...
            if(hist > 0 && prevHist <= 0) return {signal:"CALL", confidence: 87, strength: "قوية", reason: "تقاطع MACD صاعد"};
            if(hist < 0 && prevHist >= 0) return {signal:"PUT", confidence: 87, strength: "قوية", reason: "تقاطع MACD هابط"};
        }
        return null;
    }
    ...
    function strategy_MACD_Divergence(candles) {
        ...
        if(prevPrice > lastPrice && prevMACD < lastMACD && lastMACD < 0) return {signal:"CALL", confidence: 94, strength: "قوية جدا", reason: "دايفرجنس إيجابي MACD"};
        if(prevPrice < lastPrice && prevMACD > lastMACD && lastMACD > 0) return {signal:"PUT", confidence: 94, strength: "قوية جدا", reason: "دايفرجنس سلبي MACD"};
        return null;
    }
    ...
    function strategy_BullishEngulfing(candles) {
        ...
        if(prev.close < prev.open && last.close > last.open && last.open < prev.close && last.close > prev.open) {
            return {signal:"CALL", confidence: 90, strength: "قوية جدا", reason: "ابتلاع صاعد - انعكاس"};
        }
        return null;
    }
    ...
    function strategy_BearishEngulfing(candles) {
        ...
        let prev = candles[candles.length-2];
        if(prev.close > prev.open && last.close < last.open && last.open > prev.close && last.close < prev.open) {
            return {signal:"PUT", confidence: 90, strength: "قوية جدا", reason: "ابتلاع هابط - انعكاس"};
        }
        return null;
    }
    ...
    function strategy_SupportResistance(candles) {
        ...
        if(current > resistance && current > prev && body > avgBody * 1.5) return {signal:"CALL", confidence: 89, strength: "قوية", reason: "اختراق مقاومة"};
        if(current < support && current < prev && body > avgBody * 1.5) return {signal:"PUT", confidence: 89, strength: "قوية", reason: "اختراق دعم"};
        if(Math.abs(current - resistance) < 0.0005 && current < resistance) return {signal:"PUT", confidence: 85, strength: "قوية", reason: "ارتداد من مقاومة"};
        if(Math.abs(current - support) < 0.0005 && current > support) return {signal:"CALL", confidence: 85, strength: "قوية", reason: "ارتداد من دعم"};
        return null;
    }
    ...
    function strategy_DemandZoneBounce(candles) {
        ...
        if(nearestDemand && Math.abs(current - nearestDemand.price) < 0.0006 && current > nearestDemand.price) {
            return {signal:"CALL", confidence: 92, strength: "قوية جدا", reason: "ارتداد من منطقة طلب قوية"};
        }
        return null;
    }
    ...
    function strategy_SupplyZoneBounce(candles) {
        ...
        if(nearestSupply && Math.abs(current - nearestSupply.price) < 0.0006 && current < nearestSupply.price) {
            return {signal:"PUT", confidence: 92, strength: "قوية جدا", reason: "ارتداد من منطقة عرض قوية"};
        }
        return null;
    }
    ...
    function strategy_FibonacciRetracement(candles) {
        ...
        if(diffTo382 < tolerance) {
            if(trend === "UP") return {signal:"CALL", confidence: 90, strength: "قوية", reason: "ارتداد من فيبوناتشي 0.382"};
            if(trend === "DOWN") return {signal:"PUT", confidence: 90, strength: "قوية", reason: "ارتداد من فيبوناتشي 0.382"};
        }
        if(diffTo618 < tolerance && trend === "UP") return {signal:"CALL", confidence: 93, strength: "قوية جدا", reason: "ارتداد من فيبوناتشي 0.618"};
        return null;
    }
    ...
    function strategy_VolumeSpike(candles) {
        ...
        if(currentVol > avgVol * 2 && closes[closes.length-1] > closes[closes.length-2]) {
            return {signal:"CALL", confidence: 89, strength: "قوية", reason: "انفجار حجم مع صعود"};
        }
        if(currentVol > avgVol * 2 && closes[closes.length-1] < closes[closes.length-2]) {
            return {signal:"PUT", confidence: 89, strength: "قوية", reason: "انفجار حجم مع هبوط"};
        }
        return null;
    }
    ...
    function strategy_Swing_OrderBlock(candles) {
        ...
        if(nearestOB && Math.abs(current - nearestOB.price) < 0.0008) {
            if(nearestOB.type === "BULLISH" && current > nearestOB.price) {
                return {signal:"CALL", confidence: 94, strength: "قوية جدا", reason: `ارتداد من Order Block صاعد`};
            }
            if(nearestOB.type === "BEARISH" && current < nearestOB.price) {
                return {signal:"PUT", confidence: 94, strength: "قوية جدا", reason: `ارتداد من Order Block هابط`};
            }
        }
        return null;
    }
    ...
    function strategy_Position_Monthly_Trend(candles) {
        ...
        let current = closes[closes.length-1];
        if(current > ma50 && ma50 > ma100) return {signal:"CALL", confidence: 94, strength: "قوية جدا", reason: "ترتيب إيجابي للمتوسطات"};
        if(current < ma50 && ma50 < ma100) return {signal:"PUT", confidence: 94, strength: "قوية جدا", reason: "ترتيب سلبي للمتوسطات"};
        return null;
    }
    ...
    function strategy_Multi_PinBar(candles) {
        ...
        if(lowerShadow > body * 2 && upperShadow < body * 0.5) return {signal:"CALL", confidence: 88, strength: "قوية", reason: "نمط Pin Bar صاعد"};
        if(upperShadow > body * 2 && lowerShadow < body * 0.5) return {signal:"PUT", confidence: 88, strength: "قوية", reason: "نمط Pin Bar هابط"};
        return null;
    }
    ...
    function strategy_Multi_Doji_Reversal(candles) {
        ...
        if(body < (prev.high - prev.low) * 0.1 && prevBody > (prev.high - prev.low) * 0.6) {
            if(last.close > prev.close) return {signal:"CALL", confidence: 86, strength: "قوية", reason: "دوجي بعد شمعة كبيرة - انعكاس صاعد"};
            if(last.close < prev.close) return {signal:"PUT", confidence: 86, strength: "قوية", reason: "دوجي بعد شمعة كبيرة - انعكاس هابط"};
        }
        return null;
    }
    ...
    function showSignal(direction, strength, confidence, reason) {
        ...
        div.innerHTML = `
            ...
            <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:15px;">
                <div style="background:rgba(0,0,0,0.5);border-radius:20px;padding:5px 10px;"><span style="color:#aaa;font-size:10px;">الفريم</span><br><span style="color:${mc};font-size:13px;">${tf.name || currentTimeframeAuto}</span></div>
                <div style="background:rgba(0,0,0,0.5);border-radius:20px;padding:5px 10px;"><span style="color:#aaa;font-size:10px;">القوة</span><br><span style="color:#ffaa66;font-size:13px;">${strength}</span></div>
                <div style="background:rgba(0,0,0,0.5);border-radius:20px;padding:5px 10px;"><span style="color:#aaa;font-size:10px;">السعر الحالي</span><br><span style="color:${mc};font-size:13px;">${entryPrice.toFixed(5)}</span></div>
                <div style="background:rgba(0,0,0,0.5);border-radius:20px;padding:5px 10px;"><span style="color:#aaa;font-size:10px;">نقطة الدخول</span><br><span style="color:#00ffaa;font-size:13px;">${optimalEntry.toFixed(5)}</span></div>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:15px;">
                <div style="background:rgba(0,0,0,0.5);border-radius:20px;padding:5px 10px;"><span style="color:#aaa;font-size:9px;">🎯 جني ربح</span><br><span style="color:#00ffaa;font-size:11px;">${optimalTP.toFixed(5)}</span></div>
                <div style="background:rgba(0,0,0,0.5);border-radius:20px;padding:5px 10px;"><span style="color:#aaa;font-size:9px;">🛑 وقف خسارة</span><br><span style="color:#ff4466;font-size:11px;">${optimalSL.toFixed(5)}</span></div>
            </div>
            <div style="background:rgba(0,0,0,0.5);border-radius:25px;padding:10px 20px;max-width:400px;"><div style="font-size:12px;color:#fff;">${reason}</div></div>
        ...
        ui.innerHTML=`
            ...
            <div id="ui-main-content" style="padding:15px;">
                ...
                <div style="font-size:7px;color:#ffd96688;text-align:center;margin-top:12px;padding-top:8px;border-top:1px solid #ffffff11;">
                    ⚡ تحليل حقيقي كامل || تنفيذ تلقائي للصفقات ⚡
                </div>
            </div>`;
    ...
    function startAnalysis() {
        ...
        document.getElementById('status-text').innerHTML=`🟢 يعمل | ${selectedTimeframe} | تنفيذ تلقائي ${SETTINGS.autoExecuteTrades ? '✅' : '❌'}`;
        showSearchingStatus();
    }
