// ============================================
// Obeida Trading Bot - النسخة المحسّنة V4.0 ULTIMATE
// 150+ استراتيجية - كشف تلقائي كامل + رادار السعر + فيبوناتشي + مستويات ذكية
// ============================================
(function(){
    'use strict';

    // ========== كلمة المرور ==========
    const BOT_PASSWORD = "2025";
    ...
    function initPriceRadar() {
        console.log("%c 🛰️ رادار العملة الواحدة نشط.. جارٍ التصفية", "color: #00ffcc; font-weight: bold;");
    ...
    function updateFibonacciLevels() {
        ...
        // إيجاد أعلى وأدنى سعر في آخر 50 شمعة
    ...
    function getOptimalEntry(price, direction) {
        ...
        if (direction === "CALL") {
            // أفضل نقطة دخول للشراء هي عند مستويات الدعم
            ...
            // أفضل نقطة دخول للبيع هي عند مستويات المقاومة
    ...
    const STRATEGY_PERFORMANCE = {
        // استراتيجيات سكالبينج فائق السرعة (نسبة نجاح 85-95%)
        ...
        // استراتيجيات سكالبينج سريع (نسبة نجاح 80-90%)
        ...
        // استراتيجيات يومية (نسبة نجاح 75-85%)
        ...
        // استراتيجيات سوينغ (نسبة نجاح 80-90%)
        ...
        // استراتيجيات وضعية (نسبة نجاح 85-92%)
    ...
    // استراتيجية شمعة النجمة الهابطة
    ...
    // استراتيجية التقاطع الذهبي للمتوسطات
    ...
    function analyzeChart() {
        ...
        // حساب الوزن لكل إشارة حسب الثقة
    ...
    function createUI() {
        ...
        ui.id='obeida-ui';
        ui.style.cssText=`position:fixed;bottom:20px;right:20px;width:450px;
            background:linear-gradient(145deg,#0a0f1e,#020408);border-radius:25px;
            border:1px solid rgba(255,217,102,0.4);z-index:999990;direction:rtl;
            font-family:'Tahoma','Segoe UI',monospace;box-shadow:0 10px 30px rgba(0,0,0,0.5);`;
        
        ui.innerHTML=`
            <div style="background:#000000aa;padding:12px;text-align:center;border-bottom:1px solid #ffd966;border-radius:25px 25px 0 0;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="font-size:24px;">🔥</span>
                        <div>
                            <h3 style="color:#ffd966;margin:0;font-size:16px;">Obeida Trading Pro V4</h3>
                            <div style="font-size:9px;color:#88ccff;">150+ استراتيجية | فيبوناتشي | رادار لحظي</div>
                        </div>
                    </div>
                    <button id="hide-btn" style="background:none;border:none;color:#ffd966;cursor:pointer;font-size:16px;">🔼</button>
                </div>
            </div>
            <div id="ui-main" style="padding:12px;">
                ...
                <div style="background:#00000066;border-radius:12px;padding:8px;text-align:center;margin-bottom:10px;">
                    ...
                    <div style="display:flex;justify-content:center;align-items:center;gap:15px;">
                        <span id="current-price-display" style="font-size:18px;color:#00ffaa;font-weight:bold;">0.00000</span>
                        <span id="price-diff-display" style="font-size:12px;">● 0</span>
                    </div>
                </div>
                <!-- معلومات الكشف -->
                ...
                <div id="current-timeframe-display" style="background:#00000066;border-radius:12px;padding:8px;text-align:center;font-size:10px;margin-bottom:10px;line-height:1.6;"></div>
                <!-- مستويات فيبوناتشي -->
                ...
                <div style="display:flex;gap:10px;margin-bottom:10px;">
                    ...
                    <button id="stop-btn" style="flex:1;padding:10px;background:#8b2c2c;border:none;border-radius:20px;color:#fff;cursor:pointer;display:none;font-weight:bold;">⏹ إيقاف</button>
                </div>
                ...
                <div style="display:flex;gap:10px;margin-top:10px;">
                    ...
                    <button id="telegram-btn" style="flex:1;padding:6px;background:linear-gradient(95deg,#0088cc,#006699);border:none;border-radius:15px;color:#fff;cursor:pointer;font-size:10px;">📢 تليجرام</button>
                    <button id="fib-toggle" style="flex:1;padding:6px;background:#4a2a2a;border:none;border-radius:15px;color:#fff;cursor:pointer;font-size:10px;">📊 فيبوناتشي</button>
                </div>
                <div style="font-size:8px;color:#ffd966;text-align:center;margin-top:10px;">⚡ 150+ استراتيجية | فيبوناتشي | رادار لحظي | كشف تلقائي كامل ⚡</div>
            </div>`;
        ...
        if(hideBtn) hideBtn.onclick=()=>{
            let m=document.getElementById('ui-main');
            if(m){let h=m.style.display==='none';m.style.display=h?'block':'none';hideBtn.innerHTML=h?'🔽':'🔼';}
        };
    ...
    function showSettingsModal() {
        ...
        modal.innerHTML=`
            <div style="background:linear-gradient(145deg,#0a0f1e,#020408);padding:30px;border-radius:30px;border:2px solid #ffd966;width:320px;">
                ...
                <button id="save-settings" style="width:100%;padding:10px;background:linear-gradient(95deg,#ffd966,#ffaa33);border:none;border-radius:20px;color:#000;cursor:pointer;font-weight:bold;">حفظ الإعدادات</button>
                <button id="close-settings" style="width:100%;margin-top:10px;padding:8px;background:#333;border:none;border-radius:20px;color:#fff;cursor:pointer;">إغلاق</button>
            </div>`;
    ...
    function showPasswordModal() {
        ...
        modal.innerHTML=`
            <div style="background:linear-gradient(145deg,#0a0f1e,#020408);padding:40px;border-radius:50px;border:2px solid #ffd966;text-align:center;width:340px;">
                ...
                <h2 style="color:#ffd966;margin:10px 0;">Obeida Trading Pro V4</h2>
                <p style="color:#88ccff;font-size:12px;">150+ استراتيجية | فيبوناتشي | رادار لحظي</p>
                ...
                <button id="login-btn" style="width:100%;padding:12px;background:linear-gradient(95deg,#ffd966,#ffaa33);border:none;border-radius:30px;color:#000;cursor:pointer;font-weight:bold;font-size:14px;">تأكيد الدخول</button>
                <p style="color:#ffaa66;margin-top:20px;font-size:11px;">📢 للحصول على كلمة المرور:
                    <span id="tg-link" style="color:#88ccff;cursor:pointer;">@ObeidaTrading</span> على تليجرام</p>
                <div style="font-size:9px;color:#555;margin-top:15px;">⚡ 150+ استراتيجية | فيبوناتشي | رادار لحظي | كشف تلقائي كامل ⚡</div>
            </div>`;
        document.body.appendChild(modal);
    ...
    // ========== بدء التشغيل ==========
    console.log(`✨ Obeida Trading Bot V4 - 150+ استراتيجية | فيبوناتشي | رادار لحظي ✨`);
    showPasswordModal();
    ...
    window.ObeidaPro = {
        ...
        version: "V4.0 - 150+ استراتيجية + فيبوناتشي",
